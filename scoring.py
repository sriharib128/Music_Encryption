# audio file is f"./midi_files/audio_{raga}_{i}.mid
# coding:utf-8
"""core.py
...
def final_score(set1_eval):
    return int(set1_eval['total_pitch_class_histogram'][0][1]*100 + sum(set1_eval['avg_IOI']))
...
def scoring(audio_file):
    # Replicate the audio file path 10 times
    audio_files = [audio_file] * 10

    set1_eval = {'total_used_pitch': np.zeros((10, 1)), 
                 'pitch_range': np.zeros((10, 1)), 
                 'avg_IOI': np.zeros((10, 1)), 
                 'total_pitch_class_histogram': np.zeros((10, 12))}
    metrics_list = list(set1_eval.keys())

    for idx, audio_file_path in enumerate(audio_files):
        if audio_file_path.endswith(".mid"):
            feature = extract_feature(audio_file_path)
            for metric_name in metrics_list:
                set1_eval[metric_name][idx] = getattr(metrics(), metric_name)(feature)
            
    score = final_score(set1_eval)
    print("Score:", score)
    return score
